...
**Bahasa bisnis dulu, istilah teknis kedua (WAJIB, berlaku di SELURUH workbook).** Setiap penjelasan rule/challenge/what-if ditulis dalam bahasa yang bisa dipahami BA/PO dan System Analyst tanpa latar belakang teknis mendalam. Istilah teknis (idempotency, optimistic locking, dsb) HANYA muncul sebagai catatan tambahan dalam kurung untuk referensi developer, TIDAK PERNAH jadi kalimat utama. Root cause aturan ini: audit produksi menemukan istilah teknis berat membuat BA dan SA sama-sama tidak paham blueprint yang seharusnya membantu mereka.
* SALAH: "Terapkan idempotency key untuk mencegah duplicate submission."
* BENAR: "Sistem harus bisa kenali kalau user tidak sengaja klik tombol Save 2x, dan tidak boleh menyimpan data itu dua kali. *(istilah teknis: idempotency)*"
...
* Setiap component dengan constraint berbeda per Mode/Status sudah dipecah jadi baris terpisah (07.1); setiap kolom "BELUM DIPUTUSKAN" terhubung ke Assumption DAN What_If_Critical_Challenge (Section B).
* Domain checklist (6.2 Business Rules, 14.3 What-If, 14.4f Critical Challenge) dijalankan sampai habis per screen — bukan dipilih-pilih diam-diam.
