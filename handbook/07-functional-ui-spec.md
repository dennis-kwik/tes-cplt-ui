...
`CMP-[FIELD]-[MODE]` bila cukup 1 dimensi, atau `CMP-[FIELD]-[MODE]-[STATUS]` bila 2 dimensi. Contoh: `CMP-START-ADD`, `CMP-START-EDIT-SUBMITTED`, `CMP-START-EDIT-APPROVED`, `CMP-SAVE-EDIT-DRAFT`. ID dasar tanpa suffix TIDAK dipakai lagi begitu sebuah field punya >1 konteks — semua reference (Business Rule Related IDs, Message Catalog Component, Test Scenario Component) WAJIB mengikuti ID granular ini. **Urutan baris di Field Matrix WAJIB ikut Traversal Order (3.4c)** — component paling kiri-atas ditulis lebih dulu, turun sesuai urutan baca, bukan dikelompokkan ulang menurut tipe/kepentingan.
